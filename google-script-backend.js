...
/* =======================
   DEV: Reset Password (Public - Development Only)
======================= */
function devResetPassword_(body) {
  const username = body && body.username ? String(body.username).trim() : '';
  const newPassword = body && body.new_password ? String(body.new_password) : '';

  if (!username || !newPassword) {
    return err_('VALIDATION', 'نام کاربری و رمز عبور جدید الزامی است.', 400);
  }

  const users = readAll_('users');
  const target = findOne_(users, u => String(u.username) === username);
  if (!target) {
    return err_('NOT_FOUND', 'کاربر پیدا نشد.', 404);
  }

  const salt = salt16_();
  const hash = sha256Hex_(salt + newPassword);

  const lock = LockService.getScriptLock();
  lock.waitLock(15000);
  try {
    const ok = updateUserByUsername_(username, {
      password_salt: salt,
      password_hash_sha256: hash
    });

    if (ok) {
      audit_('system', 'dev_password_reset', '', '', JSON.stringify({ username }), '', '');
      return ok_({
      ...
      return err_('UPDATE_FAILED', 'خطا در به‌روزرسانی رمز عبور.', 500);
    }
  } finally {
    lock.releaseLock();
  }
...
/* =======================
   Bootstrap (optional):
   Run manually once: sets admin password to sasan
======================= */
